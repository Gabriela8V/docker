...
conexao.connect((erro) => {
  ...
  console.log(`Banco de dados online ->${conexao.threshold}`);
});
...
app.post("/usuarios/cadastro", (req, res) => {
  ...
  conexao.query("insert into usuario set ?", req.body, (error, data) => {
    if (error)
      return res.status(500).send({ output: `Erro ao cadastrar -> ${error}` });
    res.status(201).send({ output: `Usuario cadastrad`, dados: data });
  });
});
...
app.post("/usuarios/login", (req, res) => {
  ...
  conexao.query(
    ...
    [req.body.nomecompleto, req.body.senha],
    (error, data) => {
      if (error)
        return res
          .status(500)
